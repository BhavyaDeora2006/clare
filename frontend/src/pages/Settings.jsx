...
// Radio Group
const RadioGroup = ({ label, options, selectedValue, onChange, isDark }) => (
    <div className="mb-9">
        <h4 className={`text-[14.5px] font-semibold m-0 mb-4 ${
    isDark ? 'text-[#e7e5e4]' : 'text-[#292524]'
}`}>{label}</h4>
...
const SectionContainer = ({ title, subtitle, children, footer }) => (
    <div className="flex flex-col h-full">
        ...
        <div className="mb-8 pt-1">
            <h2 className="text-2xl font-bold text-stone-200 tracking-[-0.01em] m-0 mb-2">
                {title}
            </h2>
            {subtitle && (
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-3 w-full py-[13px] px-4 rounded-xl border-none cursor-pointer text-[14px] font-medium text-left transition-all duration-150 ${
                                isActive
                                    ? isDark ? 'bg-[#8a9a7b]/20 text-[#a8c090]' : 'bg-[#8a9a7b]/14 text-[#7a8a6b]'
                                    : isDark ? 'bg-transparent text-[#a8a29e] hover:bg-[#292524]/60' : 'bg-transparent text-[#57534e] hover:bg-[#f4f6f1]'
                            }`}
                        >
                            <span className={`flex ${
                                isActive
                                    ? isDark ? 'text-[#a8c090]' : 'text-[#7a8a6b]'
                                    : 'text-[#a8a29e]'
                            }`}>
                                {item.icon}
                            </span>
...
const CancelBtn = ({ onClick }) => (
    <button
        onClick={onClick}
        className="py-3 px-7 mb-4 text-[14px] font-medium text-[#57534e] border border-[#e7e5e4] rounded-xl bg-white cursor-pointer transition-colors duration-150 tracking-[0.01em] hover:bg-[#f4f6f1]"
    >
        Cancel
    </button>
);
...
useEffect(() => {
    ...
    return (
        ...
        >
            <div className="mb-8">
                <h4 className={`text-[14px] font-semibold ${isDark ? 'text-[#d6d3d1]' : 'text-[#292524]'} m-0 mb-4`}>Profile Picture</h4>
    ...
    isDark ? 'text-[#d6d3d1]' : 'text-[#292524]'
}`}>Display Name</h4>
                {!isEditingName ? (
                    ...
                    <div className="p-4 bg-white/45 border border-[#d6d3d1]/40 rounded-2xl flex flex-col gap-3 max-w-[320px]">
                        ...
                        <div className="flex justify-end gap-2 mt-1">
                            <button
                                ...
                                className="px-4 py-2 text-[13px] font-medium rounded-xl border cursor-pointer transition-colors text-[#57534e] border-[#e7e5e4] bg-white hover:bg-[#f4f6f1]"
                            >
                                Cancel
                            </button>
                            <button
...
const AppearanceSection = ({ prefs, updateField, onSave, onCancel, isDark }) => {
    return (
        <SectionContainer
            title="Appearance"
...
const StudySettingsSection = ({ prefs, updateField, onSave, onCancel, isDark }) => {
    return (
        <SectionContainer
        ...
        >
            ...
            <RadioGroup
                ...
                options={[{ value: "direct", label: "Direct", description: "Short, concise answers." }, { value: "guided", label: "Guided", description: "Step-by-step reasoning." }, { value: "exploratory", label: "Exploratory", description: "Extended conceptual links." }]}
...
const SecuritySection = ({isDark}) => {
    ...
    return (
        ...
        >
            ...
            {user && (
                <div className="mb-6">
                    <p className={`text-[14px] font-semibold m-0 mb-4 ${
    isDark ? 'text-[#d6d3d1]' : 'text-[#292524]'
}`}>Current Email</p>
                    <input
                        type="text"
                        value={user.email || ''}
                        readOnly
                        className="w-full px-4 py-3 rounded-xl border border-[rgba(214,211,208,0.9)] bg-[#f4f6f1] text-[#57534e] text-[14px] outline-none cursor-not-allowed mb-2"
                    />
    ...
    isDark ? 'text-[#d6d3d1]' : 'text-[#292524]'
                ...
                <form onSubmit={handleUpdateEmail} className="flex flex-col gap-3">
                    ...
                    <button
                        ...
                        className="w-full py-3 rounded-xl text-[14px] font-medium text-white bg-gradient-to-r bg-[#8a9a7b] hover:bg-[#7a8a6b] shadow-[0_1px_4px_rgba(0,0,0,0.14)] hover:shadow-[0_4px_12px_rgba(138,154,123,0.25)] transition-all duration-300 cursor-pointer border-none disabled:opacity-60"
                    >
    ...
    isDark ? 'text-[#d6d3d1]' : 'text-[#292524]'
                ...
                <button
                    ...
                    className="w-full py-3 rounded-xl text-[14px] font-medium text-[#57534e] bg-transparent border border-[#d6d3d1] hover:bg-[#fafaf8] transition-all duration-300 cursor-pointer disabled:opacity-60"
                >
        ...
        >
            <div className="flex flex-col mt-2">
                ...
                <button
                    ...
                    disabled={signingOut}
                    className="w-full py-4 mb-4 rounded-xl text-[14px] font-semibold text-white bg-gradient-to-r bg-[#8a9a7b] hover:bg-[#7a8a6b] shadow-[0_10px_30px_rgba(138,154,123,0.25)] hover:shadow-[0_14px_40px_rgba(138,154,123,0.35)] transition-all duration-300 cursor-pointer border-none disabled:opacity-60"
                >
                ...
                <button
                    onClick={handleSignOutAll}
                    disabled={signingOutAll}
                    className="w-full py-4 rounded-xl text-[14px] font-medium text-[#57534e] bg-transparent border border-[#d6d3d1] hover:bg-[#fafaf8] hover:border-[#a8a29e] transition-all duration-300 cursor-pointer disabled:opacity-60"
                >
                    {signingOutAll ? 'Signing out everywhere...' : 'Sign out of all devices'}
                </button>
                    ...
                    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm">
                        <div className={`rounded-2xl p-8 max-w-md w-full mx-4 shadow-xl border transition-colors duration-300 ${isDark ? 'bg-[#1c1917] border-[#44403c]/50' : 'bg-white border-transparent'}`}>
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className={`text-xl font-semibold m-0 ${isDark ? 'text-stone-200' : 'text-[#292524]'}`}>Delete your account</h3>
                            </div>
                            <p className={`text-[14px] mb-6 m-0 leading-relaxed ${isDark ? 'text-[#a8a29e]' : 'text-[#a8a29e]'}`}>
                            ...
                            <input
                                ...
                                className={`w-full px-4 py-3 border rounded-xl mb-6 outline-none focus:border-red-400 text-[14px] transition-colors ${isDark ? 'bg-[#292524]/60 border-[#44403c]/50 text-stone-200 placeholder:text-[#57534e]' : 'bg-white border-stone-300 text-[#292524]'}`}
                            />
                            <div className="flex justify-end gap-3">
                                <button
                                    onClick={() => { setShowDeleteModal(false); setDeleteInput(''); }}
                                    className={`px-5 py-2.5 text-[14px] font-medium bg-transparent border-none cursor-pointer rounded-xl transition-colors ${isDark ? 'text-[#d6d3d1] hover:bg-[#44403c]/50' : 'text-[#a8a29e] hover:bg-stone-100'}`}
                                >
                                    Cancel
                                </button>
                                <button
